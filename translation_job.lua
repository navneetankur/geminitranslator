...
local function queue_delete(handles)
  ...
  if existing == "" then
    write_file(F_TODELETE, "#!/bin/sh\n" ..
      ...
      "# these do NOT expire on their own. run when every batch is done: sh " .. F_TODELETE .. "\n" ..
      body)
  else
...
local function cmd_get(batchfile, outdir)
  ...
  if truncated == 0 and failed == 0 then
    ...
    -- results are safely on disk now, so the (non-expiring) server-side input
    -- and output files can go too. Queue both into to_delete.sh for a batch wipe.
